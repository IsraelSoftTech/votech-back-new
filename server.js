  ...
  try {
    ...
    for (const column of missingColumns) {
      ...
      if (columnExists.rows.length === 0) {
        ...
        if (column === "current_salary" || column === "expected_salary")
          columnType = "DECIMAL(10,2)";
        if (column === "created_at" || column === "updated_at")
          columnType = "TIMESTAMP DEFAULT CURRENT_TIMESTAMP";

        await pool.query(
          `ALTER TABLE applications ADD COLUMN ${column} ${columnType}`
        );
  ...
  try {
    ...
    let query = `
      SELECT i.*, 
             ...
             ac.default_depreciation_rate as category_depreciation_rate
      FROM inventory i
      LEFT JOIN budget_heads bh ON i.budget_head_id = bh.id
      LEFT JOIN asset_categories ac ON i.asset_category = ac.name
    `;
    let params = [];
    if (type) {
      query += " WHERE type = $1";
      ...
      if (monthsSincePurchase > 0) {
        ...
        await pool.query(
          `INSERT INTO asset_depreciation (
            inventory_id, asset_name, original_cost, current_value, 
            depreciation_rate, monthly_depreciation, total_depreciation, calculation_date
          ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
          ON CONFLICT (inventory_id, calculation_date) 
          DO UPDATE SET
