...
module.exports = function createDisciplineCasesRouter(pool, authenticateToken) {
  ...
  router.post("/", authenticateToken, async (req, res) => {
    const { student_id, teacher_id, class_id, case_description, case_type } =
      req.body;
    ...
    if (!student_id) {
      return res
        .status(400)
        .json({ error: "Either student_id or teacher_id is required" });
    }

    if (student_id && teacher_id) {
      return res
        .status(400)
        .json({ error: "Cannot have both student_id and teacher_id" });
    }
    ...
    try {
      ...
      if (error && error.code === "23503") {
        return res
          .status(400)
          .json({ error: "Invalid foreign key provided (class or teacher)." });
      }
  ...
  router.post("/teacher", authenticateToken, async (req, res) => {
    ...
    if (!teacher_id || !case_description) {
      return res
        .status(400)
        .json({ error: "teacher_id and case_description are required" });
    }
    try {
      ...
      if (userCheck.rows.length === 0) {
        return res.status(400).json({
          error: "Invalid teacher_id. Please select a valid teacher.",
        });
      }
      let result;
      if (class_id) {
      ...
      res
        .status(500)
        .json({ error: "Failed to create teacher discipline case" });
    }
  });
  ...
  router.put("/:id/status", authenticateToken, async (req, res) => {
    ...
    if (!["resolved", "not resolved"].includes(status)) {
      return res
        .status(400)
        .json({ error: 'Status must be "resolved" or "not resolved"' });
    }
    ...
    try {
      ...
      const checkStudent = await pool.query(
        "SELECT * FROM discipline_cases WHERE id = $1",
        [id]
      );
      ...
      const isPrivilegedRole =
        ...
        roleLower === "discipline" ||
        roleLower === "psychosocialist" ||
        roleLower === "psycho";
      if (!isPrivilegedRole && recordedBy !== req.user.id) {
        return res.status(403).json({
          error: "Access denied. You can only update cases you recorded.",
        });
      }
      ...
      const result = await pool.query(
        `UPDATE discipline_cases SET ${updateFields} WHERE id = $1 RETURNING *`,
        paramsStudent
      );

      const fieldsChanged = {};
      const old = checkStudent.rows[0];
      const updated = result.rows[0];
      if (old.status !== updated.status)
        fieldsChanged.status = { before: old.status, after: updated.status };
      if (old.resolved_at !== updated.resolved_at)
        fieldsChanged.resolved_at = {
          before: old.resolved_at,
          after: updated.resolved_at,
        };
      if (old.resolved_by !== updated.resolved_by)
        fieldsChanged.resolved_by = {
          before: old.resolved_by,
          after: updated.resolved_by,
        };
      if (old.resolution_notes !== updated.resolution_notes)
        fieldsChanged.resolution_notes = {
          before: old.resolution_notes,
          after: updated.resolution_notes,
        };
      await logChanges(
        "discipline_cases",
        id,
  ...
  router.delete("/:id", authenticateToken, async (req, res) => {
    ...
    try {
      ...
      if (!req.user.role.startsWith("Admin") && recordedBy !== req.user.id) {
        return res.status(403).json({
          error: "Access denied. You can only delete cases you recorded.",
        });
      }

      const result = await pool.query(
        "DELETE FROM discipline_cases WHERE id = $1 RETURNING *",
        [id]
      );

      await logChanges("discipline_cases", id, ChangeTypes.delete, req.user);
