...
module.exports = function createStaffAttendanceRouter(pool, authenticateToken) {
    ...
    try {
      ...
      if (constraintTest.rows.length === 0) {
        ...
        try {
          ...
          console.log('Could not add unique constraint (may already exist):', constraintError.message);
        }
      }
  ...
  router.get('/stats', authenticateToken, async (req, res) => {
    try {
      ...
      const currentStats = currentResult.rows[0] || {
        total_records: 0, present_count: 0, absent_count: 0, late_count: 0, half_day_count: 0
      };
      ...
      const currentAttendanceRate = currentStats.total_records > 0 
        ? Math.round((currentStats.present_count / currentStats.total_records) * 100)
        : 0;

      const lastAttendanceRate = lastStats.total_records > 0 
        ? Math.round((lastStats.present_count / lastStats.total_records) * 100)
        : 0;
  ...
  router.post('/records', authenticateToken, async (req, res) => {
    try {
      const { date, staff_name, time_in, time_out, classes_taught, status } = req.body;
  ...
  router.put('/records/:id', authenticateToken, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to update staff attendance record' });
    }
  });
  ...
  router.delete('/records/:id', authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      const result = await pool.query('DELETE FROM staff_attendance_records WHERE id = $1 RETURNING *', [id]);
  ...
  router.get('/monthly-report', authenticateToken, async (req, res) => {
    try {
      ...
      const fullTimeExpectedDays = parseInt(settings.full_time_expected_days) || 22;
      const partTimeExpectedDays = parseInt(settings.part_time_expected_days) || 11;
      ...
      const usersResult = await pool.query(`
        ...
        ORDER BY name
      `);
      const allStaff = [...new Set([...staffFromRecords, ...usersResult.rows.map(u => u.name)])];
      ...
      const reportData = allStaff.map(staff => {
        ...
        const expectedDays = employmentType === 'Full Time' ? fullTimeExpectedDays : partTimeExpectedDays;
        ...
        monthDays.forEach(day => {
          staffReport.daily_records[day] = {
            date: `${day.toString().padStart(2, '0')}/${month.padStart(2, '0')}/${year}`,
            status: 'Absent',
            time_in: null,
            time_out: null,
            classes_taught: '',
        ...
        staffRecords.forEach(record => {
          ...
          const minutesWorked = calculateTimeDifference(record.time_in, record.time_out);
          
          // Calculate late arrival and early departure
          const timeInMinutes = record.time_in ? timeToMinutes(record.time_in) : null;
          const timeOutMinutes = record.time_out ? timeToMinutes(record.time_out) : null;
          const lateArrivalMinutes = timeInMinutes && timeInMinutes > startTimeMinutes 
            ? timeInMinutes - startTimeMinutes 
            : 0;
          const earlyDepartureMinutes = timeOutMinutes && timeOutMinutes < endTimeMinutes 
            ? endTimeMinutes - timeOutMinutes 
            : 0;
          
          const minutesMissed = Math.max(0, expectedWorkMinutesPerDay - minutesWorked);
          const totalMinutesMissed = record.status === 'Absent' 
          ...
          switch (record.status) {
            case 'Present':
              ...
              staffReport.total_late_arrival_minutes = (staffReport.total_late_arrival_minutes || 0) + lateArrivalMinutes;
              staffReport.total_early_departure_minutes = (staffReport.total_early_departure_minutes || 0) + earlyDepartureMinutes;
              break;
            case 'Absent':
        ...
        // Calculate absent days (days not present)
        staffReport.absent_days = daysInMonth - staffReport.present_days - staffReport.late_days - staffReport.half_days;
        if (staffReport.absent_days < 0) staffReport.absent_days = 0;

        // Convert minutes to hours
        staffReport.total_hours_worked = Math.round((staffReport.total_minutes_worked / 60) * 10) / 10;
        staffReport.total_hours_missed = Math.round((staffReport.total_minutes_missed / 60) * 10) / 10;
        staffReport.total_late_arrival_hours = Math.round((staffReport.total_late_arrival_minutes / 60) * 10) / 10;
        staffReport.total_early_departure_hours = Math.round((staffReport.total_early_departure_minutes / 60) * 10) / 10;

        // Calculate attendance rate based on expected days
        staffReport.attendance_rate = expectedDays > 0 
          ? Math.round((staffReport.present_days / expectedDays) * 100)
          : 0;

        // Calculate compliance rate (actual vs expected)
        staffReport.compliance_rate = expectedDays > 0
          ? Math.round((staffReport.present_days / expectedDays) * 100)
          : 0;
      ...
      const totalExpectedDays = reportData.reduce((sum, staff) => sum + staff.expected_days, 0);
      const totalPresentDays = reportData.reduce((sum, staff) => sum + staff.present_days, 0);

      const overallAttendanceRate = totalExpectedDays > 0 
        ? Math.round((totalPresentDays / totalExpectedDays) * 100)
        : 0;
  ...
  router.get('/employment-status', authenticateToken, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to fetch staff employment status' });
    }
  });
  ...
  router.put('/employment-status', authenticateToken, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to update staff employment status' });
    }
  });
  ...
  router.put('/settings', authenticateToken, async (req, res) => {
    try {
      const { full_time_expected_days, part_time_expected_days, start_time, end_time } = req.body;

      if (full_time_expected_days !== undefined) {
        if (!Number.isInteger(full_time_expected_days) || full_time_expected_days < 1 || full_time_expected_days > 31) {
          return res.status(400).json({ 
            error: 'Full time expected days must be an integer between 1 and 31' 
          });
        }
      ...
      if (part_time_expected_days !== undefined) {
        if (!Number.isInteger(part_time_expected_days) || part_time_expected_days < 1 || part_time_expected_days > 31) {
          return res.status(400).json({ 
            error: 'Part time expected days must be an integer between 1 and 31' 
          });
        }
