...
const { pool, authenticateToken, logUserActivity, createUserSession, endUserSession, getIpAddress, getUserAgent, JWT_SECRET } = require('./utils');
...
router.post('/setup-admin', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      ['Admin User', 'admin', hashedPassword, 'admin', 'admin@school.com', 'admin@school.com']
    );
...
router.post('/register', async (req, res) => {
  try {
    ...
    const ipAddress = getIpAddress(req);
    const userAgent = getUserAgent(req);
    await logUserActivity(newUser.id, 'create', `Created user: ${username}`, 'user', newUser.id, username, ipAddress, userAgent);
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    const ipAddress = getIpAddress(req);
    const userAgent = getUserAgent(req);
    await logUserActivity(userResult.rows[0].id, 'update', 'Password reset', 'user', userResult.rows[0].id, username, ipAddress, userAgent);
...
router.post('/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    const ipAddress = getIpAddress(req);
    const userAgent = getUserAgent(req);
    await logUserActivity(req.user.id, 'update', 'Password changed', 'user', req.user.id, user.username, ipAddress, userAgent);
