...
router.put('/:id', authenticateToken, upload.fields([
  ...
  try {
    ...
    const updateQuery = `UPDATE vocational SET ${updateParts.join(', ')} WHERE id = $${paramIndex} AND user_id = $${paramIndex + 1}`;
...
router.delete('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await pool.query('DELETE FROM vocational WHERE id = $1 AND user_id = $2', [vocationalId, userId]);
